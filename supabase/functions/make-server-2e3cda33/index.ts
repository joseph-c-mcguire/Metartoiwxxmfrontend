...
const JWKS = createRemoteJWKSet(new URL("https://ktvxijislbtgqapllmuk.supabase.co/auth/v1/.well-known/jwks.json"));
...
app.post("/make-server-2e3cda33/convert/metar-to-iwxxm", async (c)=>{
  try {
    ...
    const { bulletinId = 'SAAA00', issuingCenter = 'KWBC', iwxxmVersion = '3.0', strictValidation = true, includeNilReasons = true, onError = 'warn', logLevel = 'INFO' } = params || {};
...
async function verifyProjectJWT(jwt) {
  try {
    ...
    console.log(`   JWKS URL: https://ktvxijislbtgqapllmuk.supabase.co/auth/v1/.well-known/jwks.json`);
...
app.post("/make-server-2e3cda33/admin/approve-user", async (c)=>{
  try {
    ...
    if (result.data) {
      await admin.sendEmailNotification(result.data.email, 'Your Account Has Been Approved', `Dear ${result.data.username},\n\nYour account has been approved! You can now log in to the METAR Converter application.\n\nBest regards,\nThe Admin Team`);
    }
    return c.json({
      message: 'User approved successfully',
      profile: result.data
...
app.post("/make-server-2e3cda33/admin/reject-user", async (c)=>{
  try {
    ...
    if (result.data) {
      await admin.sendEmailNotification(result.data.email, 'Account Registration Update', `Dear ${result.data.username},\n\nWe regret to inform you that your account registration has not been approved at this time.\n\nIf you have questions, please contact our support team.\n\nBest regards,\nThe Admin Team`);
    }
    return c.json({
